...
void DrawTextFT::setFontFile(const std::string &font_file) {
  ...
  if (!font_file_.empty()) {
    ...
    if (err_code != FT_Err_Ok) {
      throw std::runtime_error("could not load default font data");
    }
  }
...
// This is the font data from Telex-Regular.ttf
// The original file, along with the full text of the SIL Open Font License
//   under which it is distributed is included in $RDBASE/Data/Fonts
...
const std::string telex_ttf((const char *)raw_data,
                            (const char *)raw_data + sizeof(raw_data));
